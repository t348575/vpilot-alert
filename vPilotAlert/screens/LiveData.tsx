  ...
  useEffect(() => {
    ...
    const fetchLiveData = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setError(`Failed to fetch live data: ${err}`);
          setData(null);
        }
      }
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <View>
          <DataRow label="ETA:" value={data.eta ?? '-'} />
          ...
          <DataRow
            ...
            value={data.prev_waypoint ?? '-'}
          />
          <DataRow
            label="Route Deviation (NM):"
            value={
              data.route_deviation != null
                ? data.route_deviation.toFixed(2)
                : '-'
            }
          />
          <DataRow
            ...
            value={
              data.route_progress != null
                ? `${data.route_progress.toFixed(1)}%`
                : '-'
            }
          />
          <DataRow
            label="Distance to Next WP (NM):"
            value={
              data.dist_next_wp != null ? data.dist_next_wp.toFixed(2) : '-'
            }
          />
          ...
          <DataRow
            ...
            value={
              data.pilot
                ? `${data.pilot.latitude}, ${data.pilot.longitude}`
                : '-'
            }
          />
          <DataRow
            label="Altitude:"
            value={
              data.pilot?.altitude != null ? `${data.pilot.altitude} ft` : '-'
            }
          />
          <DataRow
            label="Ground Speed:"
            value={
              data.pilot?.groundspeed != null
                ? `${data.pilot.groundspeed} knots`
                : '-'
            }
          />
        </View>
